# -*- coding: utf-8 -*-
import logging; logging.basicConfig(level=longing.INFO)
...
def response_factory(app, handler):
    ...
    def response(request):
        ...
        if isinstance(r, dict):
            ...
            if template is None:
                resp = web.Response(body=json.dumps(r, ensure_ascii=False, default=lambda 0: 0.__dict__).encode('utf-8'))
                resp.content_type = 'application/json;charset=utf-8'
                return resp
            else:
                resp = web.Response(body=app['templating__'].get_template(template).render(**r).encode('utf-8'))
                resp.content_type = 'text/html;charset=utf-8'
                return resp
...
def init(loop):
    ...
    init_jinja2(app, filter=dict(datetime_filter))
